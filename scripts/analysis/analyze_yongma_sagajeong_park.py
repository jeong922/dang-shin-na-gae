...
for candidate in seoul_candidates:
    ...
    if not is_related_name(candidate["name"]):
        continue
...
# ============================================================
# 현재 Polygon 내부의 관련 OSM 후보
# ============================================================

print()
print("=" * 70)
print("현재 Polygon 내부의 용마 / 사가정 OSM 후보")
print("=" * 70)
...
for candidate in osm_candidates:
    ...
    # OSM 후보의 80% 이상이 현재 Polygon에
    # 들어가 있는 경우
    if candidate["candidate_overlap"] < 0.8:
        continue

    inside_related_osm.append(candidate)


if not inside_related_osm:

    print("현재 Polygon 내부에 대부분 포함되는 " "용마 / 사가정 OSM 후보가 없습니다.")


for candidate in inside_related_osm:
...
# 1. 용마 / 사가정 이름을 가짐
# 2. 대표 좌표와 가까움
# 3. 공식 면적과 너무 크게 차이나지 않음
#
# 을 함께 만족하는 후보를 우선 확인한다.
# ============================================================
